...
class STDP(LearningMethod):
    ...
    def update_trace(self, neuron):
        trace = [
            0 if x == 0 else neuron.trace[int(i)] + 1
            for (i, x) in enumerate(1 - neuron.inputs)
        ]
        return trace
        ...
        for layer in layers:
            for neuron in layer:
                if not self.is_setup:
                    self.setup(neuron)
                correlated_adjustment = (
                    self.adjustment if neuron.refractoryTime == 0 else -self.adjustment
                )
